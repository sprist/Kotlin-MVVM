...
/**
 *
 * DESC：基于Paging的ViewHolder  基类
 * Created by liyuhang on 2019/7/3
 *
 */

open class BaseViewHolder(view: View, private val adapter: RecyclerView.Adapter<BaseViewHolder>) :
    RecyclerView.ViewHolder(view) {

    private val views = SparseArray<View>()

    fun isLastItem(): Boolean {
        return layoutPosition == adapter.itemCount - 1
    }

    fun getDataPosition(): Int {
        return when (adapter) {
            is BasePagingAdapter<*> -> {
